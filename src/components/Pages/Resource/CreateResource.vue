  ...
  export default {
    ...
    computed:{
      types(){
       return [
          ...
          ...this.$store.getters.resourcesAsRef()
       ];
      }
    },
    components: {
    ...
    methods: {
      ...
      saveResource(){
        ...
        this.resource.project = projectId;

        this.$store.commit('addResource', this.resource);
    ...
    created(){
      ...
      if(projectId !== this.$store.getters.currentProject ) {
        this.$store.dispatch('bindResourcesRef', projectId)
      }
    }
  }
</script>
